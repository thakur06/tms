  ...
  if (isLoading) {
    return (
      <div className="flex justify-center items-center h-screen">
        <div className="animate-spin rounded-full h-12 w-12 border-4 border-(--primary) border-t-transparent shadow-lg shadow-(--primary-glow)"></div>
      </div>
    )
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <motion.div
          ...
          className="ui-card p-4 flex items-center gap-4 group relative overflow-hidden transition-all shadow-sm hover:shadow-xl bg-(--hover-bg) border-(--glass-border)"
        >
          ...
          <div className="relative z-10">
            <p className="text-[10px] font-black uppercase tracking-widest text-(--text-muted) group-hover:text-blue-500 transition-colors">Total Projects</p>
            <p className="text-xl font-black text-(--text-main) transition-all">{stats.total}</p>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="relative z-10 min-w-0">
            <p className="text-[10px] font-black uppercase tracking-widest text-(--text-muted) group-hover:text-rose-500 transition-colors">Closed Projects</p>
            <p className="text-xl font-black text-(--text-main) truncate transition-all">{stats.closed}</p>
          </div>
        </motion.div>
        ...
        <motion.div
          ...
          className="ui-card p-4 flex items-center gap-4 group relative overflow-hidden transition-all shadow-sm hover:shadow-xl bg-(--hover-bg) border-(--glass-border)"
        >
          <div className="absolute inset-0 bg-linear-to-br from-(--amber) via-transparent to-(--amber-glow) opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
          <div className="relative z-10 p-3 bg-(--amber-glow) text-(--amber) rounded-xl border border-(--amber-glow) shadow-sm group-hover:shadow-(--amber-glow) transition-all">
            <IoCodeSlashOutline size={24} className="group-hover:scale-110 transition-transform" />
          </div>
          <div className="relative z-10">
            <p className="text-[10px] font-black uppercase tracking-widest text-(--text-muted) group-hover:text-(--amber) transition-colors">Active Projects</p>
            <p className="text-xl font-black text-(--text-main) transition-all">{stats.active}</p>
          </div>
        </motion.div>
      </div>
      ...
      <div className="mb-8 space-y-4">
        <div className="flex flex-col xl:flex-row xl:items-end justify-between gap-4">
          <div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-(--text-muted) mt-1.5 text-xs font-bold italic">Global workspace initiative tracking</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            <select
              value={filterStatus}
              className="bg-(--input-bg) text-xs font-bold text-(--text-main) border border-(--glass-border) rounded-lg h-10 px-3 focus:outline-none focus:border-(--primary)"
            ...
            <select
              ...
              className="bg-(--input-bg) text-xs font-bold text-(--text-main) border border-(--glass-border) rounded-lg h-10 px-3 focus:outline-none focus:border-amber-500"
            ...
            {(filterStatus !== 'All' || filterLocation !== 'All') && (
              <button
                ...
                className="text-xs font-bold text-(--primary) hover:text-(--primary) px-2"
              >
                Clear Filters
              </button>
            )}
