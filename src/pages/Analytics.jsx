         ...
         <div className="relative">
            <button
               ...
               className={`w-full flex items-center justify-between px-3 py-3 bg-white/5 border rounded-xl text-xs font-medium transition-all ${
                  isOpen ? 'border-indigo-500 bg-white/10 text-white' : 'border-white/10 text-slate-400'
               }`}
            >
               <span className={selectedValues.length > 0 ? 'text-white' : 'text-slate-500'}>
                  {selectedValues.length > 0 ? `${selectedValues.length} selected` : `Select ${label}...`}
               </span>
               <IoChevronDown className={`text-indigo-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} size={14} />
            </button>
                  ...
                  >
                     ...
                     <div className="max-h-48 overflow-y-auto custom-scrollbar p-1">
                        {filtered.map(opt => (
                           ...
                           >
                              {opt}
                              {selectedValues.includes(opt) && <IoCheckmarkCircle className="text-indigo-400" size={14} />}
                           </button>
                        ))}
                        {filtered.length === 0 && (
...
export const Analytics = () => {
    ...
    const fetchBasics = async () => {
      try {
        ...
        const [usersRes, projectsRes] = await Promise.all([
          axios.get(`${server}/api/users?limit=1000`, { headers: { Authorization: `Bearer ${token}` } }),
        ...
        // Extract Depts
        const depts = [...new Set((usersRes.data.users || []).map(u => u.dept).filter(Boolean))];
        setDeptsList(depts);
      } catch (error) {
  ...
  const handleFetchDepts = async () => {
     ...
     if(data) {
        ...
        // Set Title
        if (tempSelectedDepts.length === 0) setPageTitle("All Departments Analytics");
  ...
  const analytics = useMemo(() => {
    ...
    if (selectedUsers.length > 0) {
      filteredUsers = filteredUsers.filter(u => selectedUsers.includes(u.user_name));
    }
    
    if (selectedDepts.length > 0) {
      filteredUsers = filteredUsers.filter(u => selectedDepts.includes(u.user_dept));
    }
    ...
    // 3. Compute Metrics
    const totalMinutes = allEntries.reduce((sum, e) => sum + (e.hours * 60 + e.minutes), 0);
    ...
    allEntries.forEach(e => {
      ...
      } else if (e.project?.toLowerCase().includes('pto') || e.task_id?.toLowerCase().includes('pto')) {
         cat = 'pto';
    ...
    // Unique counts
    const uniqueProjects = new Set(entriesByCat.project.map(e => e.project)).size;
    ...
    const utilization = totalCapacityHours > 0 ? ((totalMinutes / 60) / totalCapacityHours) * 100 : 0;
  ...
  }, [reportData, selectedUsers, selectedDepts, projectsList, startDate, endDate]);
    ...
    if (card.data && card.data.length > 0) {
       ...
       const aggregateData = (items, keyField) => {
          ...
          items.forEach(e => {
             ...
             const key = keyField === 'task_id' ? e.task_id : (e.project || e.task_id);
          ...
          })).sort((a,b) => (b.hours*60+b.minutes) - (a.hours*60+a.minutes)); // Sort by duration
       };
          ...
          {modalContent.data.map((item, idx) => (
             <div key={idx} className="p-3 bg-slate-50 dark:bg-slate-800 rounded-lg">
                <div className="flex justify-between">
                   <div className="font-semibold text-slate-900 dark:text-white">{item.project}</div>
                ...
                <div className="flex justify-between mt-2 text-xs text-slate-500">
                   ...
                   <div className="font-mono text-slate-900 dark:text-white">{item.hours}h {item.minutes}m</div>
                </div>
             </div>
          ))}
       </div>
      ...
      <div className="relative z-10 p-6 bg-gradient-to-r from-white/5 to-white/10 dark:from-slate-900/50 dark:to-slate-800/50 backdrop-blur-sm rounded-2xl border border-white/10 shadow-lg space-y-6">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
           ...
           <div className="bg-white/5 rounded-xl p-5 border border-white/5 flex flex-col gap-4">
             ...
             >
                {isLoading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : "Fetch Dept Data"}
             </button>
           </div>
           ...
           <div className="bg-white/5 rounded-xl p-5 border border-white/5 flex flex-col gap-4">
             ...
             >
                {isLoading ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : "Fetch User Data"}
             </button>
           </div>
        </div>
      </div>
          ...
          >
             ...
             <div className="relative p-6 bg-white/5 border border-white/10 rounded-2xl backdrop-blur-sm hover:border-indigo-500/30 transition-colors">
                <div className="flex justify-between items-start mb-4">
                   ...
                   {card.data && card.data.length > 0 && <IoArrowUp className="text-slate-500 -rotate-45 group-hover:text-white transition-colors" />}
                </div>
                <div>
               ...
               {analytics.chartData.length > 0 ? (
                  <ResponsiveContainer width="100%" height="100%">
                     <LineChart data={analytics.chartData}>
                       ...
                       <XAxis dataKey="date" stroke="#94a3b8" fontSize={12} tickFormatter={(str) => new Date(str).toLocaleDateString(undefined, {month:'short', day:'numeric'})} />
                       ...
                       />
                       <Line type="monotone" dataKey="hours" stroke="#818cf8" strokeWidth={3} dot={{r:4}} />
                     </LineChart>
                  </ResponsiveContainer>
               ) : (
                     ...
                     <PieChart>
                       ...
                       >
                         {analytics.locationData.map((entry, index) => (
                           <Cell key={`cell-${index}`} fill={['#818cf8', '#34d399', '#fbbf24', '#f87171', '#c084fc'][index % 5]} />
                         ))}
                       </Pie>
                       <Tooltip />
                       <Legend />
