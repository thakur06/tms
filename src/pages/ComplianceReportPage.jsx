...
export default function ComplianceReportPage() {
  ...
  const filteredData = useMemo(() => {
      return reportData.filter(item => {
          ...
          else if (statusFilter === 'not_submitted') matchesStatus = item.status === 'not_submitted' || item.status === 'draft'; // draft is also not submitted
    ...
    <div className="p-6 md:p-8 space-y-8 min-h-screen bg-black text-white font-sans selection:bg-amber-500/30">
        ...
        <div className="bg-zinc-900/30 border border-white/5 rounded-2xl overflow-hidden shadow-2xl min-h-[500px]">
            {loading ? (
                ...
                <ComplianceTable data={filteredData} weekDays={weekDays} onAction={() => {}} enableActions={false} />
            )}
        </div>
    </div>
  );
