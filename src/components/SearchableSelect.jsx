...
const SearchableSelect = ({
    ...
    return (
        <div
            className={`searchable-select-container flex-1 transition-all ${showLabel ? 'space-y-1.5' : ''} ${className} ${disabled ? 'opacity-50 pointer-events-none' : ''} ${isOpen ? 'relative z-50' : ''}`}
            ref={containerRef}
        >
            {showLabel && (
                ...
                <AnimatePresence>
                    {isOpen && (
                        <motion.div
                            ...
                            className="absolute top-full left-0 right-0 mt-2 border border-white/10 rounded-2xl overflow-hidden z-100 shadow-2xl bg-zinc-950 backdrop-blur-xl ring-1 ring-white/10"
                        >
