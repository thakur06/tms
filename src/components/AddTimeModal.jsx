...
import { IoTime, IoFolder, IoLocation, IoDocumentText, IoCheckmarkCircle, IoClose, IoSearch } from 'react-icons/io5'
  ...
  useEffect(() => {
    if (isOpen && entry) {
      ...
      if (entry.hours !== undefined || entry.minutes !== undefined) {
        // Entry has separate hours and minutes
        totalMinutes = (Number(entry.hours) || 0) * 60 + (Number(entry.minutes) || 0)
      } else if (entry.minutes !== undefined && entry.minutes !== null) {
        // Entry has only total minutes field
  ...
  const filteredTasks = tasks
    ? searchTask
      ? tasks.filter(t =>
        ...
        t.project?.toLowerCase().includes(searchTask.toLowerCase())
      )
      : tasks
    : []
      ...
      <div className="space-y-6">
        ...
        <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 border border-indigo-100 p-4">
          ...
          <div className="relative">
            <p className="text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-1">Logged By</p>
            <p className="text-lg font-bold text-gray-900">{selectedUser || entry?.user || 'Not specified'}</p>
          ...
          <div className="relative">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                if (selectedTask && currentTask && e.target.value !== currentTask.title) {
                  setSelectedTask('')
                }
              }}
            ...
            <div className="mt-3 p-3 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-200 rounded-xl">
              <div className="flex items-start gap-2">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold text-green-900">{currentTask.title}</p>
                  {currentTask.project && (
                    <p className="text-xs text-green-700 mt-1">Project: {currentTask.project}</p>
                  )}
                </div>
              </div>
            </div>
                  ...
                  {filteredTasks.map((task) => (
                    <button
                      ...
                      onMouseDown={(e) => {
                        e.preventDefault()
                        e.stopPropagation()
                        handleTaskSelect(task.id, task.title)
                      }}
                      disabled={isSubmitting}
                      className={`w-full text-left px-4 py-3 transition-all ${selectedTask === task.id
          ...
          {selectedProject && (
            <div className="mt-3 p-3 bg-gradient-to-r from-purple-50 to-pink-50 border-2 border-purple-200 rounded-xl">
              <div className="flex items-start gap-2">
                ...
                <p className="text-sm font-semibold text-purple-900">{selectedProject}</p>
              </div>
            </div>
          )}
            ...
            <div className="flex-1">
              <div className="relative">
                <input
                  ...
                  disabled={isSubmitting}
                />
                <p className="text-xs text-gray-500 text-center mt-2 font-medium">HOURS</p>
              </div>
            </div>
            ...
            <div className="flex-1">
              <div className="relative">
                <input
                  ...
                  disabled={isSubmitting}
                />
                <p className="text-xs text-gray-500 text-center mt-2 font-medium">MINUTES</p>
              </div>
            </div>
          </div>
          ...
          {isEditMode && entry && (
            <div className="mt-4 p-4 bg-gradient-to-r from-amber-50 to-orange-50 border-2 border-amber-200 rounded-xl">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs font-semibold text-amber-600 uppercase tracking-wider mb-1">Previous</p>
                  <p className="text-lg font-bold text-gray-900">
                    {(entry.hours || 0)}h {(entry.minutes || 0)}m
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-xs font-semibold text-indigo-600 uppercase tracking-wider mb-1">New Total</p>
        ...
        <div className="flex gap-3 pt-2">
          <button
            ...
            onClick={handleClose}
            disabled={isSubmitting}
            className={`flex-1 px-6 py-4 border-2 border-gray-300 bg-white text-gray-700 rounded-xl font-semibold hover:bg-gray-50 hover:border-gray-400 transition-all ${isSubmitting ? 'opacity-50 cursor-not-allowed' : ''
              }`}
          >
            Cancel
          </button>
          ...
          <button
            ...
            disabled={isSubmitting}
            className={`flex-1 px-6 py-4 bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600 text-white rounded-xl font-bold hover:shadow-xl hover:scale-[1.02] transition-all flex items-center justify-center gap-2 ${isSubmitting ? 'opacity-70 cursor-not-allowed' : ''
              }`}
          >
            {isSubmitting ? (
              <>
